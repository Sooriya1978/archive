...
source('enunciados_questionario2.R')
source('localfunctions.R')

weekend<-read.csv('data/weekend_outside_library.csv')
weekdays<-read.csv('data/weekdays_outside_library.csv')

for (i in questionario2)
{
  num = i[[1]]
  pergunta = i[[2]]
  options = i[[3]]
  ...
  nomepdf = paste('output/questionario2','pergunta',num,'.pdf',sep="")
  pdf(nomepdf)
  ...
  nomecsv = paste('output/questionario2','pergunta',num,'.csv',sep="")
